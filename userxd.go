...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present Userxd will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()

	t := os.Getenv("UserxdMAXPROCS")
	...
	if version {
		fmt.Println("Userxd", __version__)
		os.Exit(0)
	}
	...
	go func() {
		fmt.Println("-- Userxd ATTACK STARTED! Bu RaconunTarihini Biz Yazdık Onu Uygalayacak Olanda Biziz!!! LoL --\n           Go!\n\n")
		...
		for {
			...
			case targetComplete:
				sent++
				fmt.Printf("\r%-6d of max %-6d |\t%7d |\t%6d", cur, maxproc, sent, err)
				fmt.Println("\r-- Userxd Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
